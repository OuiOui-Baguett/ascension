...
import {
  ...
  coinBurst, floatText, bumpShake, slotWinFx, showEmote, setCinematic, type Spot,
} from './scene';
import { initAudio, play, setAmbient, stopAmbient, toggleMute, isMuted } from './audio';
import { FLOORS, floorAt, ITEMS, itemAt, fmt, CARD_NAMES, SKIN_COLORS, HATS, EMOTES, MAX_PLAYERS } from '../../shared/content';
...
function renderGame() {
  ...
  if (myCrash) {
    ...
    const RULES: any = {
      ...
      ROULETTE: 'Mise sur une couleur. Rouge et noir paient ×2, le vert ×36.',
      ...
      CHESTS: '9 caisses, une seule à ouvrir. La plupart sont vides.',
      PLINKO: 'La bille dévale les picots. Les cases des bords paient ×18.',
    };
    if (occ) {
      ...
      if (m.archetype === 'ROULETTE') {
        ...
        action = `<div class="lbl">Choisis une caisse — 5 sur 9 sont vides</div>
        ...
        const verb = m.archetype === 'CRASH' ? 'ENGAGER' : m.archetype === 'BLACKJACK' || m.archetype === 'HILO' ? 'DISTRIBUER' : m.archetype === 'SLOTS' ? 'LANCER' : 'MISER';
