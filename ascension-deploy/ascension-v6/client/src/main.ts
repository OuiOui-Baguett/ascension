// ============================================================
// Client — monde 3D, Coffre central, boutique, mise saisie à la main.
// ============================================================
import { net } from './net';
import {
  ...
  coinBurst, floatText, bumpShake, vaultFx, type Spot,
} from './scene';
...
let vaultAmt = 0, panel: 'NONE' | 'VAULT' | 'SHOP' = 'NONE';
...
net.on('ev', m => {
  ...
  switch (m.kind) {
    ...
    case 'withdraw': case 'deposit': vaultFx(m.kind === 'deposit'); break;
...
function clampBet(v: number) {
  return Math.max(S.betMin, Math.min(S.betMax, Math.min(Math.round(v || 0), wallet())));
}
function ensureBet() {
...
function renderBriefing() {
  ...
  ui.innerHTML = `
    <div class="screen soft">
      ...
      <p class="tip">🏦 Retire des jetons au <b>Coffre</b> (au centre) pour jouer, puis reviens <b>déposer</b> tes gains : seule la cagnotte compte pour le Péage.</p>
    </div>`;
  bankShown = S.bank;
}
...
function bindBetPad(after: () => void) {
  ...
  ui.querySelectorAll('.quick button').forEach(b => (b as HTMLElement).onclick = () => {
    ...
    betVal = clampBet(q === 'min' ? S.betMin : q === 'x2' ? betVal * 2 : q === 'half' ? betVal / 2 : wallet());
    after();
  });
}
...
function renderGame() {
  ...
  if (myCrash) {
    ...
    bottom = shopPanel();
  } else if (spot?.kind === 'VAULT') {
    bottom = `<div class="actionbar">
      <div class="abhead"><b>🏦 LE COFFRE</b><small class="mdesc">Cagnotte ${fmt(S.bank)}</small></div>
      <button class="go" id="openvault">OUVRIR LE COFFRE</button></div>`;
  } else if (spot?.kind === 'SHOP') {
    bottom = `<div class="actionbar">
      <div class="abhead"><b>🛒 BOUTIQUE</b><small class="mdesc">Payée avec tes jetons en main</small></div>
    ...
    } else if (wallet() < S.betMin) {
      bottom = `<div class="actionbar"><div class="abhead"><b>${icon[m.archetype]} ${m.name}</b></div>
        <div class="locked">Il te faut ${fmt(S.betMin)} en main — va au 🏦 Coffre.</div></div>`;
    } else {
      ensureBet();
      let action = '';
    ...
    bottom = `<div class="hint">🕹️ Va au 🏦 Coffre retirer des jetons, puis vers une machine</div>`;
  }
  ...
  const ov = $('#openvault'); if (ov) ov.onclick = () => { panel = 'VAULT'; vaultAmt = 0; render(); };
...
function vaultPanel(): string {
  const step = S.betMin * 10;
  if (!vaultAmt) vaultAmt = Math.min(S.bank, step);
  return `<div class="actionbar">
    <div class="abhead"><b>🏦 LE COFFRE</b><button class="close" id="closepanel">✕</button></div>
    <div class="vrow mono"><span>Cagnotte</span><b>${fmt(S.bank)}</b></div>
    <div class="vrow mono"><span>En main</span><b>${fmt(wallet())}</b></div>
    <div class="betpad">
      <button class="stepb" id="vminus">−</button>
      <input class="betin mono" id="vin" type="number" inputmode="numeric" value="${Math.round(vaultAmt)}">
      <button class="stepb" id="vplus">+</button>
    </div>
    <div class="hilobtns">
      <button class="hb wd" id="vwd">⬇️ RETIRER</button>
      <button class="hb dp" id="vdp">⬆️ DÉPOSER</button>
    </div>
    <button class="go" id="vall">⬆️ TOUT DÉPOSER (${fmt(wallet())})</button>
  </div>`;
}
function bindVault() {
  const inp = $('#vin') as HTMLInputElement | null;
  if (!inp) return;
  const step = S.betMin * 10;
  inp.oninput = () => { vaultAmt = Math.max(0, Math.round(Number(inp.value) || 0)); };
  $('#vminus')!.onclick = () => { vaultAmt = Math.max(0, vaultAmt - step); render(); };
  $('#vplus')!.onclick = () => { vaultAmt = vaultAmt + step; render(); };
  $('#vwd')!.onclick = () => net.send({ t: 'withdraw', amount: vaultAmt });
  $('#vdp')!.onclick = () => net.send({ t: 'deposit', amount: vaultAmt });
  $('#vall')!.onclick = () => net.send({ t: 'deposit' });
}
...
function shopPanel(): string {
  return `<div class="actionbar shoppanel">
    ...
    <div class="shopfoot mono">En main : <b>${fmt(wallet())}</b></div>
  </div>`;
}
function bindShop() {
...
function topBar(): string {
  ...
  const total = S.bank + S.players.reduce((s: number, p: any) => s + (p.wallet || 0), 0);
  const canElev = S.phase === 'FLOOR' && total >= S.toll;
  ...
  return `<div class="top">
    ...
    <div class="row">
      ...
      <span class="wal mono" id="wal">✋ ${fmt(wallet())}</span>
    </div>
...
function softUpdate() {
  ...
  const w = $('#wal'); if (w) w.textContent = `✋ ${fmt(wallet())}`;
  const side = $('.side'); if (side) side.outerHTML = sideList();
  if (S.phase === 'FLOOR') {
    const total = S.bank + S.players.reduce((s: number, p: any) => s + (p.wallet || 0), 0);
    if ((total >= S.toll) !== !!$('#elev')) return render();
    if (panel !== 'NONE') return render();           // montants du coffre / boutique
