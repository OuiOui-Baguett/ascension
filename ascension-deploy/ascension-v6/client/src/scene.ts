// ============================================================
// Monde 3D — Coffre central, boutique, 4 machines par étage,
// avatars personnalisables. La 3D EST l'interface.
// ============================================================
...
const NEAR_V = 4.2;      // le Coffre (gros objet, zone généreuse)
...
export const VAULT_POS = new THREE.Vector3(0, 0, -0.4);
export const SHOP_POS = new THREE.Vector3(-6.6, 0, 3.6);
const YAWS = [0.6, 0.2, -0.2, -0.6];

export type Spot = { kind: 'MACHINE'; index: number } | { kind: 'VAULT' } | { kind: 'SHOP' };
...
let vaultRoot: THREE.Group, vaultLid: THREE.Mesh, vaultGlow: THREE.PointLight;
...
let drums: { g: THREE.Group; stopAt: number; target: number; spinning: boolean }[] = [];
...
let chestAnim = { active: false, at: 0, pick: 0, reveal: [] as number[] };
...
export function nearestSpot(): Spot | null {
  ...
  const rv = a.g.position.distanceTo(VAULT_POS) / NEAR_V;
  if (rv < bestR) { bestR = rv; best = { kind: 'VAULT' }; }
...
function spotPos(i: number) { return MACHINE_SPOTS[i] ?? VAULT_POS; }
...
export function vaultFx(up: boolean) {
  coinBurst(-1, 14, VAULT_POS);
  vaultGlow.intensity = up ? 90 : 60;
}
...
function buildSlots(f: FloorDef, d: MachineDef) {
  ...
  const cab = new THREE.Mesh(new THREE.BoxGeometry(2.1, 2.6, .9), mat('#241c28'));
  cab.position.y = 1.6;
  const foot = new THREE.Mesh(new THREE.BoxGeometry(1.4, .4, .8), mat('#191320'));
  foot.position.y = .2;
  const top = new THREE.Mesh(new THREE.SphereGeometry(.18, 12, 12), mat(f.theme.accent, { emissive: f.theme.accent, emissiveIntensity: .6 }));
  top.position.y = 3.1;
  g.add(cab, foot, top);
  drums = [];
  for (let dd = 0; dd < 3; dd++) {
    ...
    for (let i = 0; i < 5; i++) {
      const face = new THREE.Mesh(new THREE.BoxGeometry(.52, .42, .05), mat(sym[i].color, { roughness: .4 }));
      const a = (i / 5) * Math.PI * 2;
      face.position.set(0, Math.sin(a) * .34, Math.cos(a) * .34);
      face.rotation.x = -a;
      drum.add(face);
    }
    drum.position.set(-.62 + dd * .62, 2, .45);
    g.add(drum);
    drums.push({ g: drum, stopAt: 0, target: 0, spinning: false });
  }
  const fr = new THREE.Mesh(new THREE.BoxGeometry(2, .08, .1), mat('#e6b64c', { metalness: .6 }));
  fr.position.set(0, 2.32, .72);
  const fr2 = fr.clone(); fr2.position.y = 1.68;
  g.add(fr, fr2, nameSprite(d.name, f));
  return g;
}
...
function buildChests(f: FloorDef, d: MachineDef) {
  const g = new THREE.Group();
  const table = new THREE.Mesh(new THREE.BoxGeometry(2.4, .9, 1.6), mat('#241c28'));
  table.position.y = .45; g.add(table);
  chestMeshes = [];
  for (let i = 0; i < 9; i++) {
    const col = i % 3, row = Math.floor(i / 3);
    const box = new THREE.Mesh(new THREE.BoxGeometry(.52, .3, .42),
      mat(f.theme.accent, { roughness: .8 }));
    (box.material as THREE.MeshStandardMaterial).color.offsetHSL(0, -.1, -.1);
    box.position.set((col - 1) * .68, 1.05, (row - 1) * .5);
    box.userData.home = box.position.clone();
    g.add(box); chestMeshes.push(box);
    const lab = textSprite(String(i + 1), '#fff', 40);
    lab.scale.set(.35, .09, 1); lab.material.depthTest = true;
    lab.position.set(box.position.x, 1.22, box.position.z);
    g.add(lab);
  }
  g.add(nameSprite(d.name, f, 3.2));
  return g;
}
...
// ---------- Coffre & boutique ----------
function buildVault(f: FloorDef) {
  const g = new THREE.Group();
  const base = new THREE.Mesh(new THREE.CylinderGeometry(1.5, 1.7, .5, 12), mat('#241c28', { metalness: .4 }));
  base.position.y = .25;
  const body = new THREE.Mesh(new THREE.CylinderGeometry(1.15, 1.3, 1.1, 12), mat('#2f2740', { metalness: .5, roughness: .5 }));
  body.position.y = 1;
  vaultLid = new THREE.Mesh(new THREE.CylinderGeometry(1.2, 1.2, .22, 12), mat('#e6b64c', { metalness: .85, roughness: .25 }));
  vaultLid.position.y = 1.66;
  const dial = new THREE.Mesh(new THREE.TorusGeometry(.35, .07, 8, 18), mat('#e6b64c', { metalness: .9, roughness: .2 }));
  dial.rotation.x = Math.PI / 2; dial.position.y = 1.8;
  // pile de jetons décorative
  for (let i = 0; i < 7; i++) {
    const chip = new THREE.Mesh(new THREE.CylinderGeometry(.16, .16, .05, 14), mat(i % 2 ? '#e6b64c' : '#e05c5c', { metalness: .6 }));
    chip.position.set(Math.cos(i) * .75, 1.8 + i * .05, Math.sin(i) * .75);
    g.add(chip);
  }
  vaultGlow = new THREE.PointLight('#e6b64c', 30, 12);
  vaultGlow.position.y = 2.4;
  const label = textSprite('LE COFFRE', '#ffe9a8', 44);
  label.scale.set(2.1, .52, 1); label.position.y = 3.1;
  g.add(base, body, vaultLid, dial, vaultGlow, label);
  g.position.copy(VAULT_POS);
  return g;
}
...
export function setFloor(index: number) {
  ...
  if (vaultRoot) scene.remove(vaultRoot);
  ...
  vaultRoot = buildVault(f); shopRoot = buildShop(f);
  propsGroup = buildProps(f);
  scene.add(vaultRoot, shopRoot, propsGroup);
}
...
export function chestsOpen(pick: number, reveal: number[], openMs: number) {
  chestAnim = { active: true, at: performance.now() + openMs * .8, pick, reveal };
}
...
function tick(now: number) {
  ...
  if (me) {
    ...
    if (len > .05) {
      ...
      const solids = [...MACHINE_SPOTS.map(s => ({ p: s, r: 1.6 })), { p: VAULT_POS, r: 1.9 }, { p: SHOP_POS, r: 1.6 }];
  ...
  marker.visible = !!sp;
  if (sp) {
    const p = sp.kind === 'MACHINE' ? MACHINE_SPOTS[sp.index] : sp.kind === 'VAULT' ? VAULT_POS : SHOP_POS;
  ...
  if (chestMeshes.length && chestAnim.active && now >= chestAnim.at) {
    chestAnim.active = false;
    chestMeshes.forEach((b, i) => {
      const m = chestAnim.reveal[i] ?? 0;
      const mm = b.material as THREE.MeshStandardMaterial;
      if (i === chestAnim.pick) {
        b.position.y = b.userData.home.y + .45;
        mm.color.set(m > 0 ? '#e6b64c' : '#3a3145');
        mm.emissive.set(m > 0 ? '#5a4310' : '#000');
      } else {
        mm.color.set(m > 0 ? '#4d6b52' : '#2b2533');
        b.position.y = b.userData.home.y - .08;
      }
    });
    setTimeout(() => {
      for (const b of chestMeshes) {
        b.position.copy(b.userData.home);
        const mm = b.material as THREE.MeshStandardMaterial;
        mm.emissive.set('#000');
        mm.color.set(new THREE.Color(floorAt(currentFloor).theme.accent).offsetHSL(0, -.1, -.1));
      }
    }, 1600);
  }
  ...
  if (vaultLid) {
    vaultLid.rotation.y += dt * .35;
    vaultGlow.intensity += (26 - vaultGlow.intensity) * Math.min(1, dt * 2);
  }
  if (propsGroup) {
