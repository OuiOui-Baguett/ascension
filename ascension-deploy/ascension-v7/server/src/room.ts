...
export interface Player {
  ...
  net: number;             // bilan personnel : gains - mises
...
export class GameRoom {
  ...
  addPlayer(name: string, ws: WebSocket, color = 0, hat = 0): Player | string {
    ...
    const p: Player = {
      ...
      net: 0, play: null, busyMachine: null, refunds: 0,
  ...
  start(p: Player) {
    ...
    for (const pl of this.players.values()) { pl.net = 0; pl.refunds = 0; }
  ...
  buy(p: Player, itemId: string) {
    ...
    this.bank -= price;                       // achat : sortie de cagnotte, pas une perte de jeu
