...
function buildWheel(f: FloorDef): THREE.Group {
  const g = new THREE.Group();
  const segs = f.machines[0].wheel!.segments;
  ...
  g.add(ptr, nameSprite(f.machines[0].name, f));
  return g;
}

function buildCrash(f: FloorDef): THREE.Group {
  const g = new THREE.Group();
  ...
  g.add(runner, nameSprite(f.machines[1].name, f));
  return g;
}

function buildSlots(f: FloorDef): THREE.Group {
  const g = new THREE.Group();
  const symbols = f.machines[2].slots!.symbols;
  // meuble
  ...
  g.add(frame, frame2, nameSprite(f.machines[2].name, f));
  return g;
}

function buildHilo(f: FloorDef): THREE.Group {
  const g = new THREE.Group();
  ...
  g.add(cardMesh, nameSprite(f.machines[3].name, f));
  return g;
}
...
export function setFloor(index: number) {
  ...
  machineRoots = [buildWheel(f), buildCrash(f), buildSlots(f), buildHilo(f)];
  machineRoots.forEach((g, i) => {
    g.position.copy(SPOTS[i]);
    g.rotation.y = YAWS[i];
    scene.add(g);
  });
  flashLight.position.copy(SPOTS[1]).setY(3);
...
export function crashCashFx() { crashMult = null; coinBurst(1); runnerX = 0; }
