...
export class GameRoom {
  ...
  addPlayer(name: string, ws: WebSocket): Player | string {
    ...
    const p: Player = {
      ...
      x: -1.5 + this.players.size * 1.1, z: 2.6, moved: true,
    };
  ...
  private applyLoss(bet: number): { lost: number; saved: number } {
  ...
  bet(p: Player, machineId: string, amount: number) {
  ...
  private playWheel(p: Player, m: MachineDef, bet: number) {
    ...
    setTimeout(() => {
      ...
      if (win > 0) {
        ...
        const { lost, saved } = this.applyLoss(bet);
  ...
  private bust(p: Player) {
    ...
    p.play = null;
    const m = this.machine(play.machineId)!;
    const { lost, saved } = this.applyLoss(play.bet);
    this.stats.biggestLoss = Math.max(this.stats.biggestLoss, lost);
