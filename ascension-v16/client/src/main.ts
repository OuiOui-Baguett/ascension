...
import {
  ...
  minesReveal, minesReset, raceRunTo, gfxInfo, gfxQuality, setJukeboxPlaying, type Spot,
} from './scene';
import { initAudio, play, setAmbient, stopAmbient, toggleMute, isMuted, setStation, stopRadio, flushStation } from './audio';
import {
  FLOORS, floorAt, ITEMS, itemAt, fmt, CARD_NAMES, SKIN_COLORS, HATS, EMOTES, MAX_PLAYERS,
  minesMult, raceOdds, STATIONS, stationAt,
} from '../../shared/content';
...
let panel: 'NONE' | 'SHOP' | 'MACHINE' | 'RADIO' = 'NONE';
let stationOn = '';          // station effectivement en cours de lecture
...
addEventListener('pointerdown', () => { initAudio(); flushStation(); }, { once: true });
addEventListener('keydown', () => { initAudio(); flushStation(); }, { once: true });
...
function applyState(state: any) {
  const prev = S?.phase, prevFloor = S?.floor, prevStation = S?.station;
  S = state;
  // radio : commune à la table, elle ne se coupe pas entre deux étages
  if (state.station !== prevStation || (prevFloor !== state.floor && state.station === 'auto')) {
    applyRadio(state.station, state.floor, state.phase);
  }
...
/** Une seule porte d'entrée pour le son d'ambiance : station choisie, étage, phase. */
function applyRadio(station: string, floor: number, phase: string) {
  const st = station || 'auto';
  const playing = phase === 'FLOOR' || phase === 'BRIEFING' || phase === 'ELEVATOR';
  if (!playing) { stopRadio(); stopAmbient(); stationOn = ''; setJukeboxPlaying(false); return; }
  if (stationOn === st) return;
  stationOn = st;
  setStation(st, floor);
  setJukeboxPlaying(st !== 'off');
}
...
function renderLobby() {
  ...
  ui.innerHTML = `
    <div class="screen lobby">
      <button class="backbtn" id="leave">← Quitter la table</button>
  ...
  // retour à l'accueil : on oublie la session, le serveur nous retire du salon à la déconnexion
...
function renderGame() {
  ...
  if (myCrash) {
    ...
    const cur = S.station || 'auto';
    bottom = `<div class="actionbar radiopanel">
      <div class="abhead"><b>📻 RADIO DU CASINO</b><button class="close" id="closepanel">✕</button></div>
      <div class="stations">
        ${STATIONS.map(st => `<button class="st${st.id === cur ? ' on' : ''}" data-s="${st.id}">
          <span class="sti">${st.icon}</span>
          <span class="stn"><b>${st.name}</b><small>${st.tag}</small></span>
          ${st.id === cur ? '<span class="stp">▶</span>' : ''}
        </button>`).join('')}
      </div>
      <div class="hintline">Tout le monde à la table entend la même station.</div></div>`;
  } else if (spot?.kind === 'RADIO') {
    const st = stationAt(S.station || 'auto');
    bottom = `<div class="actionbar">
      <div class="abhead"><b>📻 RADIO</b><small class="mdesc">${st.icon} ${esc(st.name)}</small></div>
      <button class="go" id="openradio">CHOISIR UNE STATION</button></div>`;
  ...
  const orad = $('#openradio'); if (orad) orad.onclick = () => { panel = 'RADIO'; render(); };
  ui.querySelectorAll('.st').forEach(b => (b as HTMLElement).onclick = () => {
    initAudio(); play('tap');
    net.send({ t: 'radio', station: (b as HTMLElement).dataset.s });
  });
...
(window as any).__gfx = gfxInfo;
(window as any).__gfxq = gfxQuality;
// Sonde visuelle : déclenche une animation de machine sans jouer, pour les contrôles d'image.
(window as any).__fx = { roulette: rouletteSpinTo, bj: bjShow, mines: minesReveal, race: raceRunTo, chests: chestsOpen };
(window as any).__dbg = () => ({ joined, connected, code: S?.code, phase: S?.phase, panel, keys: [...keys], input: inputVec(), canWalk: canWalk(), station: S?.station, stationOn });
