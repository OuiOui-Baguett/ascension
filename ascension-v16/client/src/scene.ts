...
// À l'écart des machines : sa zone d'approche (3,2 m) ne doit voler la fiche d'aucune
// machine — c'est ce qui arrivait quand il était à 3 m de la Course.
export const RADIO_POS = new THREE.Vector3(7.4, 0, 4.4);
const YAWS = [0.45, 0.27, 0.09, -0.09, -0.27, -0.45];

export type Spot = { kind: 'MACHINE'; index: number } | { kind: 'SHOP' } | { kind: 'RADIO' };

let renderer: THREE.WebGLRenderer, scene: THREE.Scene, camera: THREE.PerspectiveCamera;
let composer: EffectComposer | null = null, bloom: UnrealBloomPass | null = null;
let quality: 'high' | 'low' = 'high';
let fpsFrames = 0, fpsStart = 0, qualityLocked = false;
...
let jukeRoot: THREE.Group | null = null;
let jukeBars: THREE.Mesh[] = [];
let jukePlaying = false;
let currentFloor = 0, shake = 0, tNow = 0;
let cine = false;          // caméra de l'écran-titre : travelling lent autour du casino
let lookAt: number | null = null;   // ?look=N : caméra fixe devant une machine (contrôle visuel)
...
let rouletteBall: THREE.Mesh | null = null, rouletteDisc: THREE.Group | null = null;
let roulWin: THREE.Mesh | null = null;
let roul = { active: false, start: 0, dur: 0, target: 0, color: '#ffffff' };
...
export function nearestSpot(): Spot | null {
  ...
  const rr = a.g.position.distanceTo(RADIO_POS) / NEAR_R;
  if (rr < bestR) { bestR = rr; best = { kind: 'RADIO' }; }
...
function textSprite(text: string, color = '#fff', size = 42): THREE.Sprite {
  ...
  const spTex = new THREE.CanvasTexture(c); spTex.colorSpace = THREE.SRGBColorSpace;
  const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: spTex, transparent: true, depthTest: false }));
...
/** Texture canvas déclarée en sRGB — obligatoire pour tout ce qui porte une couleur. */
function srgbTex(cv: HTMLCanvasElement): THREE.CanvasTexture {
...
export function showEmote(id: string, emoji: string) {
  ...
  const sp = new THREE.Sprite(new THREE.SpriteMaterial({
    map: srgbTex(c), transparent: true, depthTest: false,
  }));
  sp.name = 'emote';
  sp.scale.set(.8, .8, 1);
...
function symbolTexture(emoji: string, bg: string): THREE.MeshStandardMaterial {
  ...
  return new THREE.MeshStandardMaterial({ map: srgbTex(c), roughness: .45 });
}
...
function buildHilo(f: FloorDef, d: MachineDef) {
  ...
  cardCtx = cv.getContext('2d')!; cardTex = srgbTex(cv); drawCard('?');
...
/** Plateau de roulette dessiné en UNE texture : 25 cases alternées, séparateurs et numéros.
 *  L'ancienne version empilait 25 disques coplanaires qui se battaient à l'affichage. */
function rouletteFaceTex() {
  return canvasTex('roulface', 512, (c, S) => {
    const R = S / 2;
    c.translate(R, R);
    for (let i = 0; i < 25; i++) {
      const a0 = (i / 25) * Math.PI * 2 - Math.PI / 2, a1 = ((i + 1) / 25) * Math.PI * 2 - Math.PI / 2;
      c.fillStyle = i === 0 ? '#1f8a4c' : (i % 2 ? '#b33636' : '#15111c');
      c.beginPath(); c.moveTo(0, 0); c.arc(0, 0, R * .98, a0, a1); c.closePath(); c.fill();
      // séparateur métallique
      c.strokeStyle = '#cfd4dd'; c.lineWidth = S * .008;
      c.beginPath(); c.moveTo(0, 0); c.lineTo(Math.cos(a0) * R * .98, Math.sin(a0) * R * .98); c.stroke();
      // numéro, tête vers l'extérieur
      c.save();
      c.rotate((a0 + a1) / 2 + Math.PI / 2);
      c.fillStyle = '#f5f0e6';
      c.font = `700 ${Math.round(S * .052)}px -apple-system, system-ui, sans-serif`;
      c.textAlign = 'center'; c.textBaseline = 'middle';
      c.fillText(String(i), 0, -R * .84);
      c.restore();
    }
    // moyeu
    c.fillStyle = '#3a2f1a'; c.beginPath(); c.arc(0, 0, R * .3, 0, Math.PI * 2); c.fill();
    c.strokeStyle = '#e6b64c'; c.lineWidth = S * .014;
...
function buildRoulette(f: FloorDef, d: MachineDef) {
  ...
  g.add(podium(f, 2.0));
  // pied et cuvette
  const foot = new THREE.Mesh(new THREE.CylinderGeometry(.36, .64, 1.1, 16),
    new THREE.MeshStandardMaterial({ map: woodTex('#3d2617', '#1d1108'), roughness: .45, metalness: .15 }));
  foot.position.y = .9;
  const bowl = new THREE.Mesh(new THREE.CylinderGeometry(1.72, 1.3, .55, 32),
    new THREE.MeshStandardMaterial({ map: woodTex('#4a2f1c', '#241408'), roughness: .4, metalness: .2 }));
  bowl.position.y = 1.7;
  g.add(foot, bowl);

  // plateau tournant : un seul disque texturé, aucune facette coplanaire
  const disc = new THREE.Group();
  const face = new THREE.Mesh(new THREE.CircleGeometry(1.34, 64),
    new THREE.MeshStandardMaterial({ map: rouletteFaceTex(), roughness: .38, metalness: .25 }));
  face.rotation.x = -Math.PI / 2;
  const cross = new THREE.Mesh(new THREE.CylinderGeometry(.13, .2, .34, 14), mat('#e6b64c', GOLD));
  cross.position.y = .18;
  for (let i = 0; i < 2; i++) {
    const bar = new THREE.Mesh(new THREE.BoxGeometry(1.5, .05, .09), mat('#e6b64c', GOLD));
    bar.position.y = .26; bar.rotation.y = (i / 2) * Math.PI;
    disc.add(bar);
  }
  disc.add(face, cross);
  disc.position.y = 2.0;
  ...
  // couronne dorée et piste de la bille
  const rim = new THREE.Mesh(new THREE.TorusGeometry(1.5, .13, 14, 48), mat('#e6b64c', GOLD));
  rim.rotation.x = Math.PI / 2; rim.position.y = 2.02;
  const lip = new THREE.Mesh(new THREE.CylinderGeometry(1.66, 1.5, .3, 40, 1, true),
    mat('#c9a24a', { metalness: .8, roughness: .3, side: THREE.DoubleSide }));
  lip.position.y = 2.1;
  g.add(rim, lip);

  rouletteBall = new THREE.Mesh(new THREE.SphereGeometry(.095, 16, 14),
    mat('#fdfaf2', { roughness: .1, metalness: .2 }));
  rouletteBall.position.set(1.44, 2.16, 0);
  // case gagnante : allumée à la couleur du tirage, sous la bille
  roulWin = new THREE.Mesh(new THREE.CircleGeometry(.26, 20),
    mat('#ffffff', { emissive: '#ffffff', emissiveIntensity: 0, transparent: true, opacity: .95 }));
  roulWin.rotation.x = -Math.PI / 2; roulWin.position.set(0, 2.045, 0);
  roulWin.visible = false; roulWin.userData.noShadow = true;
  g.add(rouletteBall, roulWin);

  // tapis de mise : rouge / noir / vert, posé à plat devant
  const stand = new THREE.Mesh(new THREE.BoxGeometry(2.2, .9, .9),
    new THREE.MeshStandardMaterial({ map: woodTex('#3d2617', '#1d1108'), roughness: .45, metalness: .15 }));
  stand.position.set(0, .45, 1.9);
  const cloth = new THREE.Mesh(new THREE.BoxGeometry(2.3, .08, 1),
    new THREE.MeshStandardMaterial({ map: velvetTex('#1f6b45'), roughness: .98 }));
  cloth.position.set(0, .94, 1.9);
  g.add(stand, cloth);
  ['#b33636', '#15111c', '#1f8a4c'].forEach((col, i) => {
    const box = new THREE.Mesh(new THREE.BoxGeometry(.62, .05, .66), mat(col, { roughness: .55 }));
    box.position.set(-.72 + i * .72, .99, 1.9);
    g.add(box);
  });
  g.add(nameSprite(d.name, f, 5.1));
  return g;
}

/** Une main de blackjack dessinée au canvas : total en gros, rôle en dessous. */
function bjCanvas(): [CanvasRenderingContext2D, THREE.CanvasTexture] {
  ...
  return [c.getContext('2d')!, srgbTex(c)];
}
function bjDraw(ctx: CanvasRenderingContext2D, tex: THREE.CanvasTexture, label: string, sub: string) {
  ctx.fillStyle = '#efe9dc'; ctx.fillRect(0, 0, 256, 356);
  ctx.strokeStyle = '#a89257'; ctx.lineWidth = 10; ctx.strokeRect(10, 10, 236, 336);
  ctx.fillStyle = '#1a1524'; ctx.font = '800 130px -apple-system, system-ui, sans-serif';
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(label, 128, 160);
  ctx.font = '700 40px -apple-system, system-ui, sans-serif'; ctx.fillStyle = '#6f6579';
  ctx.fillText(sub, 128, 288);
...
function buildBJ(f: FloorDef, d: MachineDef) {
  ...
  // Les deux mains, présentées sur un chevalet face au joueur. Matière MATE et sans
  // source lumineuse rapprochée : une carte blanche + un projecteur + le halo = un rectangle brûlé.
  [bjPCtx, bjPTex] = bjCanvas(); [bjDCtx, bjDTex] = bjCanvas();
  const easel = new THREE.Mesh(new THREE.BoxGeometry(2.5, 1.72, .12), mat('#120f1a', { roughness: .9 }));
  easel.position.set(0, 2.5, -.16); easel.rotation.x = .12;
  const frame = new THREE.Mesh(new THREE.BoxGeometry(2.62, 1.84, .06), mat('#c9a24a', GOLD));
  frame.position.set(0, 2.5, -.22); frame.rotation.x = .12;
  for (const sx of [-1, 1]) {
    const leg = new THREE.Mesh(new THREE.CylinderGeometry(.05, .05, 1.5, 8), mat('#c9a24a', GOLD));
    leg.position.set(sx * 1.1, 1.75, -.3);
    g.add(leg);
  }
  const cardMat = (t: THREE.Texture) => new THREE.MeshStandardMaterial({
    map: t, roughness: .96, metalness: 0, side: THREE.DoubleSide,
  });
  const pc = new THREE.Mesh(new THREE.PlaneGeometry(.94, 1.3), cardMat(bjPTex!));
  pc.position.set(-.6, 2.5, -.07); pc.rotation.x = .12;
  const dc = new THREE.Mesh(new THREE.PlaneGeometry(.94, 1.3), cardMat(bjDTex!));
  dc.position.set(.6, 2.5, -.07); dc.rotation.x = .12;
  g.add(easel, frame, pc, dc, nameSprite(d.name, f, 5.1));
  bjShow('—', '—');
  return g;
}
...
function diceFace(n: number) {
  ...
  return new THREE.MeshStandardMaterial({ map: srgbTex(c), roughness: .35 });
}
...
function buildMines(f: FloorDef, d: MachineDef): THREE.Group {
  ...
  g.add(podium(f, 2.05));
  // caisson incliné
  // ⚠ l'inclinaison doit être POSITIVE : à -0,34 la console tournait le dos au joueur,
  // qui ne voyait que le dessous du caisson.
  const TILT = .36;
  const body = new THREE.Mesh(new THREE.BoxGeometry(3.2, .5, 2.4),
    new THREE.MeshStandardMaterial({ map: woodTex('#3a2412', '#1b0f06'), roughness: .4, metalness: .2 }));
  body.position.set(0, 1.5, -.1); body.rotation.x = TILT;
  const tray = new THREE.Mesh(new THREE.BoxGeometry(3.0, .06, 2.24),
    new THREE.MeshStandardMaterial({ map: marbleTex('#15121f', '#4a4266'), roughness: .3, metalness: .45 }));
  tray.position.set(0, 1.5 + .28 * Math.cos(TILT), -.1 + .28 * Math.sin(TILT)); tray.rotation.x = TILT;
  g.add(tray);
  const rim = new THREE.Mesh(new THREE.BoxGeometry(3.36, .1, 2.56),
    mat(f.theme.accent, { emissive: f.theme.accent, emissiveIntensity: 1.6, metalness: .6 }));
  rim.position.set(0, 1.28, -.02); rim.rotation.x = TILT; rim.userData.noShadow = true;
  const pillar = new THREE.Mesh(new THREE.BoxGeometry(2.3, 1.3, 1.1),
    new THREE.MeshStandardMaterial({ map: woodTex('#33210f', '#180d05'), roughness: .45, metalness: .15 }));
  pillar.position.y = .78;
  // panneau arrière : donne une silhouette et porte l'enseigne
  const back = new THREE.Mesh(new THREE.BoxGeometry(3.2, 1.5, .18), mat('#171320', { roughness: .8 }));
  back.position.set(0, 2.35, -1.25); back.rotation.x = -.1;
  const backGlow = new THREE.Mesh(new THREE.BoxGeometry(2.7, .9, .06),
    mat(f.theme.accent, { emissive: f.theme.accent, emissiveIntensity: .75 }));
  backGlow.position.set(0, 2.4, -1.14); backGlow.rotation.x = -.1; backGlow.userData.noShadow = true;
  g.add(pillar, body, rim, back, backGlow);

  mineTiles = [];
  const tile = new THREE.BoxGeometry(.44, .1, .40);
  // Repère du plan incliné : un point local (h au-dessus du plateau, dz vers l'avant) tombe
  // en (y = CY + h·cos − dz·sin, z = CZ + h·sin + dz·cos). Sans ce calcul, les rangs
  // s'enfoncent dans le caisson et une seule rangée dépasse.
  const CY = 1.5, CZ = -.1, CS = Math.cos(TILT), SN = Math.sin(TILT);
  const onPlane = (h: number, dz: number) => ({ y: CY + h * CS - dz * SN, z: CZ + h * SN + dz * CS });
  for (let i = 0; i < 25; i++) {
    const col = i % 5, row = Math.floor(i / 5);
    const dz = .92 - row * .46;
    const x = (col - 2) * .56;
    const p = onPlane(.40, dz);
    const t = new THREE.Mesh(tile, mat('#9a92b8', { roughness: .4, metalness: .45 }));
    t.position.set(x, p.y, p.z);
    t.rotation.x = TILT;
    t.userData.base = p.y;
    mineTiles.push(t); g.add(t);
    // liseré sous la case : une case sombre sur un caisson sombre serait invisible
    const q = onPlane(.335, dz);
    const edge = new THREE.Mesh(new THREE.BoxGeometry(.5, .03, .46),
      mat(f.theme.accent, { emissive: f.theme.accent, emissiveIntensity: .6 }));
    edge.position.set(x, q.y, q.z);
    edge.rotation.x = TILT; edge.userData.noShadow = true;
    g.add(edge);
  }
  // trois charges factices posées sur le rebord, pour dire le danger
  for (let k = 0; k < 3; k++) {
    const bomb = new THREE.Mesh(new THREE.SphereGeometry(.16, 14, 12), mat('#15121c', { metalness: .75, roughness: .28 }));
    bomb.position.set(-1.86, 1.5 + .42 * Math.cos(TILT) - (.5 - k * .34) * Math.sin(TILT),
      -.1 + .42 * Math.sin(TILT) + (.5 - k * .34) * Math.cos(TILT));
    const fuse = new THREE.Mesh(new THREE.CylinderGeometry(.022, .022, .2, 6),
      mat('#e05c5c', { emissive: '#e05c5c', emissiveIntensity: 3 }));
    fuse.position.copy(bomb.position).add(new THREE.Vector3(0, .17, 0));
    fuse.userData.noShadow = true;
    g.add(bomb, fuse);
  }
...
/** JUKE-BOX — le poste de radio de la salle. Arche lumineuse, disque qui tourne,
 *  et un égaliseur qui bouge dès qu'une station est en marche. */
function buildJukebox(f: FloorDef): THREE.Group {
  const g = new THREE.Group();
  const wood = new THREE.MeshStandardMaterial({ map: woodTex('#43230f', '#201006'), roughness: .35, metalness: .2 });
  const body = new THREE.Mesh(new THREE.BoxGeometry(2.3, 2.5, 1.15), wood);
  body.position.y = 1.25;
  const foot = new THREE.Mesh(new THREE.BoxGeometry(2.5, .35, 1.35), mat('#1a1420', { metalness: .5, roughness: .4 }));
  foot.position.y = .17;
  // dôme vitré du haut
  const dome = new THREE.Mesh(new THREE.CylinderGeometry(1.15, 1.15, 1.5, 24, 1, true, -Math.PI / 2, Math.PI),
    new THREE.MeshPhysicalMaterial({
      color: '#2a2038', roughness: .1, metalness: .1, transmission: .5,
      transparent: true, opacity: .55, side: THREE.DoubleSide,
    }));
  dome.position.set(0, 3.15, 0); dome.rotation.y = Math.PI; dome.userData.noShadow = true;
  const cap = new THREE.Mesh(new THREE.SphereGeometry(1.16, 24, 12, 0, Math.PI * 2, 0, Math.PI / 2), mat('#e6b64c', GOLD));
  cap.position.y = 3.9;
  g.add(foot, body, dome, cap);

  // arches de néon sur les flancs
  for (const sx of [-1, 1]) {
    const arch = new THREE.Mesh(new THREE.TorusGeometry(.95, .075, 10, 26, Math.PI),
      mat(f.theme.accent, { emissive: f.theme.accent, emissiveIntensity: 3.4 }));
    arch.position.set(sx * 1.14, 2.4, 0);
    arch.rotation.set(0, Math.PI / 2, 0);
    arch.userData.noShadow = true;
    g.add(arch);
  }
  // disque vinyle visible sous le dôme
  const disc = new THREE.Mesh(new THREE.CylinderGeometry(.82, .82, .05, 40), mat('#0d0b12', { roughness: .35, metalness: .3 }));
  disc.position.set(0, 3.05, 0); disc.name = 'disc';
  const lbl = new THREE.Mesh(new THREE.CylinderGeometry(.3, .3, .06, 24), mat('#e6b64c', GOLD));
  lbl.position.set(0, 3.08, 0);
  g.add(disc, lbl);

  // façade : grille du haut-parleur et égaliseur
  const grille = new THREE.Mesh(new THREE.BoxGeometry(1.8, .8, .06), mat('#15121c', { roughness: .9 }));
  grille.position.set(0, .95, .6);
  g.add(grille);
  for (let i = 0; i < 9; i++) {
    const slat = new THREE.Mesh(new THREE.BoxGeometry(1.7, .045, .02), mat('#2e2740'));
    slat.position.set(0, .68 + i * .07, .64);
    g.add(slat);
  }
  jukeBars = [];
  for (let i = 0; i < 11; i++) {
    const bar = new THREE.Mesh(new THREE.BoxGeometry(.11, 1, .05),
      mat(f.theme.light, { emissive: f.theme.light, emissiveIntensity: 2.6 }));
    bar.position.set(-.72 + i * .145, 1.95, .6);
    bar.scale.y = .1; bar.userData.noShadow = true;
    jukeBars.push(bar); g.add(bar);
  }
  const label = textSprite('📻 RADIO', '#ffe9a8', 40);
  label.scale.set(2, .5, 1); label.position.set(0, 4.6, 0);
  g.add(label);
...
/** L'égaliseur ne s'agite que si une station tourne : le juke-box dit ce qu'il fait. */
export function setJukeboxPlaying(on: boolean) { jukePlaying = on; }
...
function buildProps(f: FloorDef) {
  ...
  g.add(bar(f, 12.4, 6.6, -1.15));   // à l écart : il barrait le passage vers les machines de droite
...
export function initScene(canvas: HTMLCanvasElement) {
  ...
  // ?quality=low|high force le réglage (utile aux tests et aux vieux téléphones)
  const lk = new URLSearchParams(location.search).get('look');
  if (lk !== null) lookAt = lk === 'shop' ? -1 : lk === 'radio' ? -2 : Number(lk);
  const q = new URLSearchParams(location.search).get('quality');
  if (q === 'low' || q === 'high') { quality = q; qualityLocked = true; }
  if (quality === 'low') applyLowQuality();
...
/** Repli automatique : si l'appareil n'arrive pas à suivre, on sacrifie le halo et les
 *  ombres plutôt que de laisser le jeu ramer. Décidé une seule fois, après 2 secondes. */
function applyLowQuality() {
  quality = 'low';
  renderer.shadowMap.enabled = false;
  renderer.setPixelRatio(1);
  composer = null; bloom = null;
  scene.traverse(o => { const m = o as THREE.Mesh; if ((m as any).isMesh) { m.castShadow = false; m.receiveShadow = false; } });
}
function watchFps(now: number) {
  ...
  if (el > 2000) {
    qualityLocked = true;
    if (fpsFrames / (el / 1000) < 24) applyLowQuality();
  }
}
export function gfxQuality() { return quality; }
...
export function setFloor(index: number) {
  ...
  if (jukeRoot) { scene.remove(jukeRoot); disposeTree(jukeRoot); jukeRoot = null; jukeBars = []; }
  ...
  rouletteBall = null; rouletteDisc = null; roulWin = null; bjPTex = null; bjDTex = null; dice = []; chestMeshes = []; plinkoBalls = []; plinkoDrops = [];
  ...
  jukeRoot = buildJukebox(f);
  shade(shopRoot); shade(propsGroup); shade(jukeRoot);
  scene.add(shopRoot, propsGroup, jukeRoot);
}
...
export function rouletteSpinTo(color: string, spinMs: number) {
  ...
  roul = {
    active: true, start: performance.now(), dur: spinMs,
    target: -((idx + .5) / 25) * Math.PI * 2,
    color: color === 'GREEN' ? '#3fe08a' : color === 'RED' ? '#ff5a5a' : '#c9c2d6',
  };
  if (roulWin) { roulWin.visible = false; (roulWin.material as THREE.MeshStandardMaterial).emissiveIntensity = 0; }
}
...
function resetTile(t: THREE.Mesh) {
  ...
  mm.color.set('#9a92b8'); mm.emissive.set('#000000'); mm.emissiveIntensity = 0;
...
function tick(now: number) {
  tNow = now;
  watchFps(now);
  // plafond large : à 10 images/s, un plafond de 0,05 divisait la vitesse de marche par deux.
  // Le déplacement est découpé en sous-pas pour que la collision reste fiable.
  const dt = Math.min(.12, (now - lastT) / 1000 || .016);
  lastT = now;
  ...
  if (me) {
    ...
    if (len > .05) {
      ...
      const total = WALK * Math.min(1, len) * dt;
      const steps = Math.max(1, Math.ceil(total / .3));
      const solids = [...MACHINE_SPOTS.map(s => ({ p: s, r: 2.05 })), { p: SHOP_POS, r: 1.7 }, { p: RADIO_POS, r: 1.25 }, ...propSolids];
      for (let k = 0; k < steps; k++) {
        // On RABOTE la part du déplacement qui entre dans l'obstacle, au lieu d'annuler
        // le pas : sans ça le joueur se colle au canapé et ne peut plus le contourner.
        let mx = dir.x * (total / steps), mz = dir.y * (total / steps);
        for (const s of solids) {
          const px = me.g.position.x - s.p.x, pz = me.g.position.z - s.p.z;
          const dist = Math.hypot(px, pz);
          if (dist > s.r + .5 || dist < .0001) continue;
          const nx = px / dist, nz = pz / dist;      // normale sortante
          const into = mx * nx + mz * nz;            // négatif = on rentre dedans
          if (into < 0) { mx -= into * nx; mz -= into * nz; }
        }
        me.g.position.x += mx; me.g.position.z += mz;
        const d = Math.hypot(me.g.position.x, me.g.position.z);
        if (d > ARENA_R) { me.g.position.x *= ARENA_R / d; me.g.position.z *= ARENA_R / d; }
        // filet de sécurité : si on est quand même dedans, on ressort au plus court
        for (const s of solids) {
          const px = me.g.position.x - s.p.x, pz = me.g.position.z - s.p.z;
          const dist = Math.hypot(px, pz);
          if (dist < s.r && dist > .0001) {
            me.g.position.x = s.p.x + (px / dist) * s.r;
            me.g.position.z = s.p.z + (pz / dist) * s.r;
          }
        }
      }
      me.g.rotation.y = Math.atan2(dir.x, dir.y) + Math.PI;
      me.moving = 1;
  ...
  if (lookAt !== null) {
    // Contrôle visuel : plein cadre sur une machine, vue du joueur.
    const t = lookAt === -1 ? SHOP_POS : lookAt === -2 ? RADIO_POS : (MACHINE_SPOTS[lookAt] ?? MACHINE_SPOTS[0]);
    const d = new THREE.Vector3(t.x, 0, t.z).normalize();     // depuis le centre de la salle
    camera.position.set(t.x - d.x * 6.4, 4.2, t.z - d.z * 6.4);
    camera.lookAt(new THREE.Vector3(t.x, 2.1, t.z));
  } else if (cine) {
  ...
  marker.visible = !!sp;
  if (sp) {
    const p = sp.kind === 'MACHINE' ? MACHINE_SPOTS[sp.index] : sp.kind === 'RADIO' ? RADIO_POS : SHOP_POS;
  ...
  for (const fx of mineFx) {
    ...
    if (fx.bomb) {
      ...
      mm.color.set('#0f2a18'); mm.emissive.set('#3fd06a');
      mm.emissiveIntensity = 1.1 + Math.max(0, 1.6 - age * 2.4);
      t.position.y = t.userData.base - Math.min(.055, age * .3);
      t.scale.set(.86, .55, .86);
    }
  }
  ...
  if (rouletteBall && roul.active) {
    ...
    // la bille file sur la piste haute, puis retombe vers les cases : hauteurs calées
    // sur le plateau (y ≈ 2,0) — elle passait auparavant SOUS la table, à y = 1,16.
    const a = roul.target + (1 - e) * Math.PI * 9;
    const r = 1.44 - e * .52;
    const bounce = t > .82 ? Math.abs(Math.sin((t - .82) * 46)) * (1 - e) * .5 : 0;
    rouletteBall.position.set(Math.cos(a) * r, 2.17 - e * .11 + bounce, Math.sin(a) * r);
    if (rouletteDisc) rouletteDisc.rotation.y = -roul.target * .6 - (1 - e) * Math.PI * 5;
    if (roulWin) {
      const m = roulWin.material as THREE.MeshStandardMaterial;
      roulWin.visible = t > .9;
      roulWin.position.set(Math.cos(a) * r, 2.05, Math.sin(a) * r);
      m.color.set(roul.color); m.emissive.set(roul.color);
      m.emissiveIntensity = t > .9 ? (t - .9) * 34 : 0;
    }
    if (t >= 1) roul.active = false;
  } else if (rouletteDisc) {
    rouletteDisc.rotation.y += dt * .22;                 // le plateau tourne doucement au repos
  }
  ...
  if (jukeRoot) {
    const disc = jukeRoot.getObjectByName('disc');
    if (disc && jukePlaying) disc.rotation.y += dt * 3.2;
    for (let i = 0; i < jukeBars.length; i++) {
      const target = jukePlaying
        ? .12 + Math.abs(Math.sin(now / (170 + i * 37) + i)) * (.9 - Math.abs(i - 5) * .07)
        : .06;
      const b = jukeBars[i];
      b.scale.y += (target - b.scale.y) * Math.min(1, dt * 12);
      b.position.y = 1.55 + b.scale.y * .5;
    }
  }
  if (propsGroup) {
