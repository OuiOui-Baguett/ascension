...
export const BUILD = 'v16-machines-fix';
  ...
  ws.on('message', (raw) => {
    ...
    try {
      switch (msg.t) {
        ...
        case 'mines': if (s.room && s.player) s.room.minesPick(s.player, Number(msg.cell)); break;
        case 'radio': if (s.room && s.player) s.room.setStation(s.player, String(msg.station)); break;
        ...
        case 'emote': if (s.room && s.player) s.room.emote(s.player, Number(msg.i)); break;
      }
    } catch (e) {
