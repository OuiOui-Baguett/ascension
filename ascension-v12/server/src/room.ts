...
import {
  ...
  betMin, betMax, floorFloat, tollOf, fmt, MAX_PLAYERS, minesMult, raceOdds,
  type MachineDef,
} from '../../shared/content';
...
function spawnSpot(i: number): { x: number; z: number } {
  ...
  return { x: -2.4 + col * 1.6 + row * 0.8, z: 1.2 - row * 1.5 };  // rangs décalés : personne pile derrière un autre
}
