...
for (const f of FLOORS) {
  ...
  r.bank = 1e9;
  const opts = { ROULETTE: 'RED', CRAPS: 'SEVEN', CHESTS: '4', RACE: '1' };
  for (const m of f.machines) {
    ...
    if (m.archetype === 'CRASH') { await wait(120); r.cashout(p); }
      ...
      // on ouvre jusqu'à 3 cases puis on encaisse ; si on saute, le coup est déjà réglé
      for (let k = 0; k < 3 && p.play; k++) r.minesPick(p, k);
